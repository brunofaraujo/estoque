...
export class ItemsService {
  ...
  async isValidOperation(supplyId: number, type: string, amount: number) {
    const current: number = (await this.prisma.supply.findUnique({where: {id:supplyId}})).current
    if (amount && type === 'I') {
      return true
    }
    if (amount && type === 'O') {
  ...
  async createMove(createMoveDto: CreateMoveDto) { //Now receiving the supplyId (Maybe check by ItemId)
    try {
      if (await this.isValidOperation(createMoveDto.supplyId, createMoveDto.type, createMoveDto.amount)) {
        let result: number;
        const current: number = (await this.prisma.supply.findUnique({where: {id: createMoveDto.supplyId}})).current
        if (createMoveDto.type === 'I') {
          result = current + createMoveDto.amount
          await this.prisma.supply.update({where: {id: createMoveDto.supplyId}, data: {current: result}})
